...
#' @param color Vector of colors for groups. Defaults to viridis scale.
...
#' @param path Name of the output file. Defaults to "spiro.svg"
...
spirograph <- function(
  ...
  rotation = 0,
  start_angle = 0,
  points_per_polygon = round(abs(1000 * windings),0),  rule = "evenodd",
  openfile = TRUE,
  aspect_ratio = 1,
  end_at_beginning = F,
  savefile = TRUE,
  ...
  if (savefile) {
    ...
    if (color_groups > 1) {
      ...
      d_transitions <- dplyr::mutate(
        d_transitions,
        color_id = if_else(transition,
                           lag(color_id, default = 1),
                           color_id),
        color_cycle_id = if_else(transition,
                                 lag(color_cycle_id,
                                     default = 1),
                                 color_cycle_id),
        id = if_else(transition,
                     lag(id, default = 1),
                     id))
...
if (end_at_beginning) {
  ...
  d <- dplyr::arrange(dplyr::bind_rows(d,
    ...
    nd <- nest(group_by(d, color_cycle_id, color_id))
    nd <- mutate(nd, col = scales::alpha(colors[color_id],
                                         alpha = transparency))
    nd <- rename(nd, x = data)

    purrr::pwalk(select(nd, x, col), polypath,
                 border = NA,
                 rule = rule)
