...
In this example, we create 6 copies of a spirograph, give each copy a different color, shift the x-coordinates, and then rotate them.

```{r rainbowsequence}
spiro(file = "rainbowsequence.svg", xlim = c(-15,15), ylim = c(-15,15)) %>% 
  image_merge(output = "rainbowsequence.svg", copies = 6) %>% 
...
The `image_spin` function takes file name of an svg file created by the `spiro` function and adds an `animateTransform` node to the .svg. To spin clockwise, set the `rpm` parameter to a postive number. To spin counter-clockwise, set the `rpm` parameter to a negative number.
...
```{r spiro_data, dev="svg", fig.width=7, fig.height=7, out.height=700, out.width=700}
library(ggplot2)
library(dplyr)
d <- spiro(
  fixed_radius = 37,
