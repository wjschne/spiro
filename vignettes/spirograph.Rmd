...
Software to make digital spirographs is not hard to find, but I had several reasons to make my own software using R:
...
spiro(
  ...
  add_background_gradient_radial(
...
spiro(
  ...
  add_lines(colors = c("gray", "black", "gray", "black"),
            line_width = 0.1) %>%
  add_background_gradient_radial(
    colors = c("black",
               "purple4", 
...
spiro(
  ...
  add_background_color(color = "gray10") 
...
spiro(
  ...
  add_lines(colors = "black", line_width = 4)
```
...
You can set the background using the pipe (`%>%`) operator and the `add_background_color`
...
spiro(
  ...
  add_background_color(color = "gray80")
```

## Rounded Corners

You can round the corners if you wish. Rounding ranges from 0 to 1, with 0 being a square and 1 being a circle.
...
spiro(
  ...
  add_background_color(color = "gray80", rounding = 0.1)
```

## Radial Gradients

If you want a radial gradient background, use the `add_background_gradient_radial` function. You can set the colors and the stops. If you do not set the stops manually, they will be evenly spaced from 0 to 1.
...
```{r}
merge_image(
  input = c("spiroblue.svg", "spirored.svg"),
  output = "redblue.svg") 
```

It is possible to merge more than two images:
...
c(
  ...
  merge_image(output = "merged_s123.svg") 
```

# Re-scaling Images

You can change the size of the image with the `scale_image` function. 
...
c(
  ...
  merge_image(output = "heptagon_petals_merged.svg")
...
```{r}
hsv_v <- rep(c(1.1,0),10)  + rep(c(-1,1),10) * seq(0.3,0.8,length.out = 20)

spiro(fixed_radius = 6,
      cycling_radius = -1,
      pen_radius = 1,
      file = "aster.svg",
      rotation = pi / 6,
      points_per_polygon = 500) %>%
  merge_image(output = "olso_aster.svg", copies = 20) %>%
  add_fills(colors = hsv(seq(0,.95,0.05), v = rep(c(0.5,1),10),
                         s = hsv_v)) %>%
  scale_image(scale = seq(1, 0, length.out = 21)[-21]) %>% 
...
spiro(file = "olso_aster.svg",
      rotation = pi / 6,
      points_per_polygon = 100) %>%
  merge_image(output = "olso_aster.svg", copies = n) %>%
  add_fills(colors = oslo_colors) %>%
  scale_image(scale = sqrt(0.75 ^ (seq(0, n - 1)))) %>%
  spin_image(rpm = 1:n + 1) %>%
  add_background_color(color = "black", rounding = 1)
```
...
The `spin_image` function takes file name of an svg file created by the `spiro` function and adds an `animateTransform` node to the .svg. To spin clockwise, set the `rpm` parameter to a postive number. 
...
spiro(
  ...
  animate_pathdot(duration = 2) %>%
  animate_pathdot(duration = 3) %>%
  animate_pathdot(duration = 4) %>%
  animate_pathdot(duration = 5) %>%
  animate_pathdot(duration = 6) 
```
