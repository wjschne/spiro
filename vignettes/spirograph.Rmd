...
tibble::tibble(
  ...
  image_merge(output = "spiralstar.svg", 
              delete_input = TRUE) %>%
  add_lines(colors = c(rep(NA,k - 1), "gray")) %>% 
  image_rotate(degrees = (1:k / 2.5)) %>% 
  add_background_gradient(
    colors = c("#FFFFFF","#26588E", "#E5E3F9","#283568", "#C8C3F3"),
    radius = 1, 
    rounding = 1, 
...
c(
  ...
  image_merge(output = "merged_s123.svg") 
```
...
spiro(fixed_radius = 6,
      ...
      rotation = pi / 6,
      points_per_polygon = 500) %>%
  image_merge(output = "olso_aster.svg", copies = 20) %>%
...
spiro(file = "olso_aster.svg",
      rotation = pi / 6,
      points_per_polygon = 100) %>%
  image_merge(output = "olso_aster.svg", copies = n) %>%
